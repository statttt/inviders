...
Shader "CustomMaterialMultiply" {
   Properties {
      _MultColor ("Multiply Material Color", Color) = (1,1,1,1) 
   ...
   SubShader {
    ...
    Cull Off
         ...
         half4 frag(vertexOutput input) : COLOR
         {
            ...
            half3 finalColor = lerp(unity_FogColor.rgb, tempcol.rgb, clamp ((unity_FogEnd.x - pos) * invDiff * (unity_FogEnd.x - pos) * invDiff * (unity_FogEnd.x - pos) * invDiff, 0.0, 1.0));
            half4 col = half4(finalColor, 1.0f) * _GlobalLightIntensity * half4(_MultColor, 1.0f);
            return col;
         }
