...
Shader "CustomMaterialTexturedTranslucent" {
   Properties {
      ...
      _ScaleX ("_ScaleX", Float) = 1.0 
      _ScaleY ("_ScaleY", Float) = 1.0  
   ...
   SubShader {
    ...
    Cull Off
         ...
         half4 frag(vertexOutput input) : COLOR
         {
            ...
            half4 newcol = tex2D(_Texture,half2(input.uv.x * _ScaleX, input.uv.y * _ScaleY)).rgba * half4(tempcol,1.0);
            half pos = length(input.posWorld);
            half diff = unity_FogEnd.x - unity_FogStart.x;
            half invDiff = 1.0f / diff;
            half3 finalColor = lerp(unity_FogColor.rgb, newcol.rgb, clamp ((unity_FogEnd.x - pos) * invDiff * (unity_FogEnd.x - pos) * invDiff * (unity_FogEnd.x - pos) * invDiff, 0.0, 1.0));
            half4 col = half4(finalColor * _GlobalLightIntensity, newcol.a * _Transparency);
            //half4 col = half4(newcol.a, newcol.a,newcol.a,newcol.a);
            return col;
         }
